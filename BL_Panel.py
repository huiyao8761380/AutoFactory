...
class AutoFactoryPanel(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        row4.prop(amProperty, 'AutoMechBool', text="Auto Mech",
                  icon='TRIA_DOWN' if amProperty.AutoMechBool else 'TRIA_RIGHT')
        ...
        if amProperty.AutoMechBool:
            ...
            if amProperty.RenderImageBool == True:
                ...
                row3a.template_icon_view(amProperty, "GenMechEnum", show_labels=True, scale=5,
                                         scale_popup=4.0)  # row3a.prop(amProperty, "GenMechEnum")
            else:
                row3g.prop(amProperty, "GenMechFolder", text="")
            ...
            row7.operator("object.bl_genmech", text="",
                          icon='MODIFIER')  # Modifier修改器bevel17蜂巢 三角形 切换风格 的进度条  Smooth50_ToFixHole
            ...
            row7b.prop(amProperty, "FreezeTime_Bool", text="",
                       icon='FREEZE')  # √固定当前时间轴所选修改器参数 按钮 默认False 没能弄wave的波或者应用到Wave及之前的修改器 不包含显示跟渲染 这些应当去掉
            # row7b.prop(amProperty, "MOD_MIRROR_Bool",text="",icon='MOD_SHRINKWRAP')#X设置为预设让别人选择吧 将所选两个物体中其中一个设为容器并生成低模 按钮 #建议跟布尔一行
            row7b.prop(amProperty, "HideChildObj_Bool", text="",
                       icon='VIS_SEL_11' if amProperty.HideChildObj_Bool else 'VIS_SEL_01',
                       toggle=True)  # VIS_SEL_11 √设置当前对象的所有子空物体不可选/不可见 RESTRICT_SELECT_ON 布尔 operator("am.hidechildobj",text="",icon='VIS_SEL_01')#！！！
            ...
            row7b.prop(amProperty, "UnrealSize_Bool", text="",
                       icon='EVENT_U')  # √设置为ue4的模型比例，并增加摄像机距离 按钮 operator("am.unrealsize",text="",icon='EVENT_U')
            row7b.operator("am.renderscene", text="", icon='SCENE_DATA')  # 添加一个渲染环境预设 按钮

            row7a = col7.row(align=True)  # align=True横里的竖
            row7a.operator("am.addboolmodifier", text="",
                           icon='MOD_BOOLEAN')  # √选择两个物体，添加一个bool修改器在主要物体的重构网格11前（Bool_sub） 按钮#布尔位置 多 另起义行
            ...
            row7a.prop(amProperty,
                       "BoolNum")  # AddBoolModifier#√选择两个物体，添加一个bool修改器在主要物体的重构网格11前（Bool_sub） 按钮#布尔位置 多 另起义行
            ...
            row7c.prop(amProperty, "ModifiersApplyTo_Int",
                       text="Apply Modifiers")  # V仅应用重构网格（或字符标记的）11前的修改器 按钮#还有一个应用并删除子物体 #还有哟个应用到40的波

            row7d = col7.row(align=True)  # 横里的竖
            # row7d.label(icon='RENDER_ANIMATION',text="")
            row7d.operator("object.mechclean", text="",
                           icon='BRUSH_DATA')  # 4.MechClean(Edit)清理后Clean 和Apply 生成UV，关联对称 按钮
            ...
            row7d.prop(amProperty, "UVModifierApply_Bool", text="",
                       icon='MODIFIER_ON')  # 应用修改器X！！！显示修改器常用设置如bevel17 MODIFIER_OFF  。。。。#MATERIAL变换材质 布尔 SHADING_RENDERED
        ...
        row4a.prop(amProperty, 'AutoRigifyBool', text="Auto Rigify",
                   icon='TRIA_DOWN' if amProperty.AutoRigifyBool else 'TRIA_RIGHT')
        if amProperty.AutoRigifyBool:
            ...
            if 'rigify' in OpenScripts:
                ...
                row10j.prop(amProperty, 'StepRigBool', text="",
                            icon='TRIA_DOWN' if amProperty.AutoRigifyBool else 'TRIA_RIGHT')
