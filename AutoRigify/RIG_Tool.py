...
def LocConstraints(Bone, Subtarget, Influence, Space='LOCAL', Owner_Space='LOCAL', UseOffset=False, x=True, y=True,
                   z=True, ix=False, iy=False, iz=False):
    bone = Bone
            ...
            for bone in SelBone:
                ...
                for ob in sel:
                    ...
                    if (bone.name in ob.name) and ('ik' not in bone.name):
                        ob.select_set(True)
                        Rig.data.bones[
                            bone.name].select = True  # bpy.context.object.data.bones['Upperarm_l'].select=True
...
def register():
    bpy.utils.register_class(OneClickAddUE4Rig)
    bpy.utils.register_class(RemoveIKBoneGroup)
    bpy.utils.register_class(RigMechByName)


def unregister():
    bpy.utils.unregister_class(OneClickAddUE4Rig)
    bpy.utils.unregister_class(RemoveIKBoneGroup)
    bpy.utils.unregister_class(RigMechByName)
